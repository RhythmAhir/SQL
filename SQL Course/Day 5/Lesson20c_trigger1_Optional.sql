...
-- in the table which violates the constraint unless the NOVALIDATE keyword is used.
-- Try the constraint again.

ALTER TABLE trade
 ADD CONSTRAINT time_chk
   CHECK ( to_char(transaction_time,'HH24:MI') BETWEEN '04:00' AND '18:00'
       AND to_char(transaction_time, 'DY') NOT IN ('SAT','SUN')) NOVALIDATE;

-- It is not possible to insert a value that violates the constraint
INSERT INTO trade (trade_id, stock_id, broker_id, stock_ex_id, transaction_time, shares, price_total)
